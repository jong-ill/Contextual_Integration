...
%% Check Performance
SSE = sum(sum((Y-(R'*W_ridge_norm')).^2,1)); %Checked by hand. R'*W = W'*R
if SSE<eps
...
coherence_values = [0.1 0.5 -0.1 -0.5];
motion_coherence_values = coherence_values;
color_coherence_values = coherence_values;
        ...
        for k = 1:length(color_coherence_values)
            for trial = 1:trials
                for t = 1:interval
                    ...
                    color = color_coherence_values(k);
                    ctx = context(i);
                    trial_conds(n,:) = [ctx color motion];
                    stim_sign = [sign(color)...
                                 sign(motion)];
    
                    % Define the correct answer for each trial
                    correct_ans(n) = stim_sign(ctx);
                    
                    
                    % Stimulus Period is 750ms in the middle of trial
                    if t>250 && t<1000
                        % Stimuli + noise
                        color = color_coherence_values(k) + randn(1)*noise_std;
                        motion = motion_coherence_values(j)+ randn(1)*noise_std;
                        %             motion = 0;
                    ...
                    dx_dt(:,t)= (-x(:,t) + W_rec*x(:,t) + W'*r(:,t)...
                                    +noise_norm*randn(size(x,1),1))/t_bio;
...
%                 if max(y(250:end)) >=0.5
%                     choice(n) = 1; %Choose right
%                 elseif min(y(250:end)) <= -0.5
%                     choice(n) = -1; %Choose left (didn't choose right)
%                 else
%                     choice(n) = 0;
%                 end
                
                choice(n) = sign(y(end));
                performance_vec(n) = choice(n)*correct_ans(n);
                
              
                
                %                 % Simpler Estimate performance
                %                 if sign(y(interval)) >=0.5
                %                     choice(trial) = 1; %Choose right
                %                 elseif sign(y(interval)) <= -0.5
